...
class DarkRenamer:
    def __init__(self, path) -> None:
        ...
        self.dir_list = []  # pointer의 파일 목록에 대한 정보를 담기 위한 리스트
    ...
    def add_file(self, file_name):
        ...
        if os.path.isdir(file_path):
            print(f"{file_name}은 파일이 아닙니다.")
            return False
        if not os.path.isfile(file_path):
            print(f"{self.pointer} 경로에 {file_name}가 없습니다.")
            return False
        file_element = {
            'file_name': file_name,
            'file_path': file_path,
        ...
        if file_element in self.rename_list:
            print(f"{file_path}가 이미 포함되어있습니다.")
            return False
    ...
    def move_pointer(self, des):
        ...
        if des in os.listdir(self.pointer):
            if os.path.isfile(des):
                print("파일로는 이동할 수 없습니다.")
                return False
            self.pointer = os.path.join(self.pointer, des)
            return True
        print("이동할 수 없는 목적지입니다.")
        return False
    ...
    def change_exp(self, new_exp):
        ...
        print("확장자명 변경을 완료했습니다.")
        return True

    def do_cmd(self):
        print("a: 추가 / r: 제외 / m: 이동 / n: 이름변경 / e: 종료")
        cmd = print("실행할 명령을 입력하세요.>> ")
        if cmd == 'a':
            file_name = print("추가할 파일 이름을 입력하세요.>> ")
            return self.add_file(file_name)
        if cmd == 'r':
            str = print("제외할 파일의 번호를 입력하세요.>> ")
            if not str.isdigit():
                print("입력이 숫자가 아닙니다. 처음부터 다시 시도하세요.")
                return False
            index = int(str) - 1
            return self.remove_file(index)
        if cmd == 'm':
            des = print("이동할 목적지를 입력하세요.>> ")
            return self.move_pointer(des)
        if cmd == 'n':
            # 확장자 바꾸기
            # 나중에 다른 기능 추가 예정...!
            exp = print("변경할 확장자 명을 입력하세요.>> .")
